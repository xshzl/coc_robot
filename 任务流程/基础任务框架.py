...
class 基础任务(ABC):
    ...
    def __init__(self, 上下文: '任务上下文'):
        ...
        # 自动加载任务参数和状态
        self.参数 = self._加载任务参数()
        self.状态 = self._加载任务状态()

    def _加载任务参数(self):
        """从数据库加载任务参数，如果不存在则使用元数据中的默认值"""
        if not self.元数据 or not self.元数据.参数列表:
            return type('参数', (), {})()  # 返回空对象

        任务类名 = self.__class__.__name__
        数据库参数 = self.数据库.获取任务参数(self.机器人标志, 任务类名)

        # 创建参数对象
        参数对象 = type('参数', (), {})()

        # 合并默认值和数据库值
        for 参数定义 in self.元数据.参数列表:
            参数名 = 参数定义.参数名
            if 参数名 in 数据库参数:
                setattr(参数对象, 参数名, 数据库参数[参数名])
            else:
                setattr(参数对象, 参数名, 参数定义.默认值)

        return 参数对象

    def _加载任务状态(self):
        """从数据库加载任务状态，如果不存在则使用元数据中的默认值"""
        if not self.元数据 or not self.元数据.状态列表:
            return type('状态', (), {})()  # 返回空对象

        任务类名 = self.__class__.__name__
        数据库状态 = self.数据库.获取任务状态(self.机器人标志, 任务类名)

        # 创建状态对象
        状态对象 = type('状态', (), {})()

        # 合并默认值和数据库值
        for 状态定义 in self.元数据.状态列表:
            状态名 = 状态定义.状态名
            if 状态名 in 数据库状态:
                setattr(状态对象, 状态名, 数据库状态[状态名])
            else:
                setattr(状态对象, 状态名, 状态定义.默认值)
    ...
    def 保存状态(self, **状态字典):
        ...
        任务类名 = self.__class__.__name__
        # 更新状态对象
        for 键, 值 in 状态字典.items():
            setattr(self.状态, 键, 值)
        # 保存到数据库
        self.数据库.保存任务状态(self.机器人标志, 任务类名, 状态字典)
